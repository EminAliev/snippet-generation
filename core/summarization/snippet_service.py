...
class SnippetGeneration:
    ...
    def sentence_similarity(self, one_word, two_word):
        ...
        return 0 if np.isnan(cosine_distance(first_vector, second_vector)) \
            else 1 - cosine_distance(first_vector, second_vector)
    ...
    def generate_snippet(self, text=None, file_name=None, top_n=2):
        snippet = []
        if file_name:
            sentences, score = read_file(file_name)
        else:
            sentences, score = read_text(text)
